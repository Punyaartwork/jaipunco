...
Route::get('feedprofile', function() {
    return DB::select("SELECT  cards.user_id,users.name,users.profile, MAX(cards.created_at)AS date FROM cards,users where cards.user_id = users.id  GROUP BY user_id order by date DESC");

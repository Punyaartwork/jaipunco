...
Route::get('feedprofile', function() {
    return Card::join('users', 'cards.user_id', '=', 'users.id')->select('user_id','users.name','users.profile','cards.cardTime')->groupBy('user_id')->where('cardTime','>=','UNIX_TIMESTAMP(CURDATE())')->take(10)->get();
});
